...
let extractedLinks = []; // قائمة الروابط المستخرجة
let isConnected = false;
let currentStats = { sent: 0, errors: 0 };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تهيئة الأدوات المساعدة (Tooltips)
    initializeTooltips();

    // تهيئة نظام الروابط المؤقتة
    initializeTemporaryAccess();
...
function initializeSocket() {
    try {
        ...
        // أحداث الروابط المؤقتة
        socket.on('temporary_link_created', function(data) {
            displayTemporaryLink(data);
        });

        socket.on('temporary_link_expired', function(data) {
            handleTemporaryLinkExpiration(data);
        });
...
function initializeAutoReplySystem() {
    ...
    if (addAutoReplyBtn) {
        addAutoReplyBtn.addEventListener('click', addAutoReplyEntry);
    }

    // تحميل الردود التلقائية الموجودة
    loadAutoReplies();
}
...
function loadAutoReplies() {
    ...
    // إضافة رد افتراضي إذا لم تكن هناك ردود
    if (autoRepliesData.length === 0) {
        addAutoReplyEntry();
    }
}

function addAutoReplyEntry(keyword = '', reply = '') {
    const container = document.getElementById('autoRepliesContainer');
    if (!container) return;

    const entryId = 'autoReply_' + Date.now();
    const entryDiv = document.createElement('div');
    entryDiv.className = 'row mb-2 auto-reply-entry';
    entryDiv.id = entryId;

    entryDiv.innerHTML = `
        <div class="col-md-5">
            <input type="text" class="form-control form-control-sm keyword-input" 
                   placeholder="الكلمة المفتاحية" value="${keyword}">
        </div>
        <div class="col-md-6">
            <input type="text" class="form-control form-control-sm reply-input" 
                   placeholder="الرد التلقائي" value="${reply}">
        </div>
        <div class="col-md-1">
            <button type="button" class="btn btn-sm btn-outline-danger remove-reply-btn" 
                    onclick="removeAutoReplyEntry('${entryId}')">
...
function removeAutoReplyEntry(entryId) {
    ...
    // إزالة من المصفوفة
    autoRepliesData = autoRepliesData.filter(item => item.id !== entryId);

    // إضافة رد جديد إذا لم تعد هناك ردود
    if (autoRepliesData.length === 0) {
        addAutoReplyEntry();
    }
}

function collectAutoRepliesData() {
    const autoReplies = [];
    const entries = document.querySelectorAll('.auto-reply-entry');

    entries.forEach(entry => {
        const keywordInput = entry.querySelector('.keyword-input');
        const replyInput = entry.querySelector('.reply-input');

        if (keywordInput && replyInput) {
...
function handleSaveSettings(e) {
    ...
    const autoReplies = collectAutoRepliesData();
...
function updateFormFields(settings) {
    try {
        ...
        const autoReplyEnabledField = document.getElementById('autoReplyEnabled');
        if (autoReplyEnabledField) {
            autoReplyEnabledField.checked = settings.auto_reply_enabled || false;
        }

        // تحديث الردود التلقائية
        if (settings.auto_replies) {
            loadAutoRepliesFromSettings(settings.auto_replies);
        }
...
function loadAutoRepliesFromSettings(autoReplies) {
    const container = document.getElementById('autoRepliesContainer');
    if (!container) return;
    ...
    for (const [keyword, reply] of Object.entries(autoReplies)) {
        addAutoReplyEntry(keyword, reply);
    }

    // إضافة رد فارغ للإضافة الجديدة
...
// ===========================
function initializeTemporaryAccess() {
    // نموذج إنشاء رابط مؤقت
    const tempLinkForm = document.getElementById('tempLinkForm');
    if (tempLinkForm) {
        tempLinkForm.addEventListener('submit', handleCreateTemporaryLink);
    }

    // عرض الروابط المؤقتة النشطة
    fetch('/api/temporary_links')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                data.links.forEach(link => displayTemporaryLink(link));
            }
        })
        .catch(error => console.error('Error fetching temporary links:', error));
}

function handleCreateTemporaryLink(e) {
    e.preventDefault();
    ...
    if (!programId || isNaN(durationHours) || durationHours <= 0) {
        showNotification('يرجى إدخال معرف البرنامج ومدة صلاحية صحيحة (بالساعات)', 'warning');
        return;
    }

    const submitBtn = document.getElementById('createTempLinkBtn');
    const originalText = submitBtn.innerHTML;

    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>جاري الإنشاء...';

    fetch('/api/create_temporary_link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ program_id: programId, duration_hours: durationHours })
    })
    .then(response => response.json())
    .then(data => {
        submitBtn.disabled = false;
        ...
        if (data.success) {
            showNotification(data.message, 'success');
            // يمكن عرض الرابط مباشرة أو إرسال حدث عبر Socket.IO
            // displayTemporaryLink(data.link_data); // إذا كانت البيانات متوفرة هنا
        } else {
            showNotification(data.message, 'error');
        }
    })
...
function displayTemporaryLink(linkData) {
    const container = document.getElementById('temporaryLinksContainer');
    if (!container) return;

    const linkElement = document.createElement('div');
    linkElement.className = 'list-group-item d-flex justify-content-between align-items-center';
    linkElement.id = `temp-link-${linkData.id}`;
    linkElement.innerHTML = `
        <div>
            <strong>البرنامج:</strong> ${linkData.program_name || 'غير معروف'} <br>
            <strong>الرابط:</strong> <a href="${linkData.link}" target="_blank">${linkData.link}</a> <br>
            <strong>ينتهي في:</strong> <span data-expires="${linkData.expires_at}">${new Date(linkData.expires_at).toLocaleString('ar-SA')}</span>
        </div>
        <button class="btn btn-sm btn-danger delete-temp-link-btn" data-link-id="${linkData.id}">
    ...
    // إضافة مستمع حدث لحذف الرابط
    linkElement.querySelector('.delete-temp-link-btn').addEventListener('click', handleDeleteTemporaryLink);

    // بدء عداد الوقت للرابط
    startLinkTimer(linkData.id, linkData.expires_at);
}

function handleDeleteTemporaryLink(e) {
    const linkId = e.target.closest('.delete-temp-link-btn').getAttribute('data-link-id');
    if (!linkId) return;

    if (!confirm('هل أنت متأكد من حذف هذا الرابط المؤقت؟')) return;

    fetch(`/api/delete_temporary_link/${linkId}`, { method: 'DELETE' })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                showNotification(data.message, 'success');
                const linkElement = document.getElementById(`temp-link-${linkId}`);
        ...
        .catch(error => {
            console.error('Error deleting temporary link:', error);
            showNotification('خطأ في الاتصال بالخادم', 'error');
        });
}

function handleTemporaryLinkExpiration(linkId) {
    showNotification('انتهت صلاحية أحد الروابط المؤقتة.', 'warning');
    const linkElement = document.getElementById(`temp-link-${linkId}`);
    if (linkElement) {
        linkElement.innerHTML = `
            <div class="text-muted d-flex justify-content-between align-items-center w-100">
                <span>انتهت صلاحية هذا الرابط</span>
                <button class="btn btn-sm btn-outline-secondary" data-link-id="${linkId}" onclick="removeExpiredLink(event)">
                    <i class="fas fa-times"></i> إزالة
                </button>
            </div>
        `;
        // إيقاف المؤقت
        const timerId = `timer-${linkId}`;
        if (window[timerId]) {
...
function removeExpiredLink(event) {
    const linkId = event.target.closest('button').getAttribute('data-link-id');
    const linkElement = document.getElementById(`temp-link-${linkId}`);
    if (linkElement) linkElement.remove();
}


function startLinkTimer(linkId, expiresAt) {
    const timerId = `timer-${linkId}`;
    const expiresElement = document.querySelector(`#temp-link-${linkId} [data-expires]`);
    ...
    window[timerId] = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(window[timerId]);
            handleTemporaryLinkExpiration(linkId);
            return;
        }
        ...
        expiresElement.textContent = `${hours} س ${minutes} د ${seconds} ث`;
    }, 1000);
}

// =========================== 
// دوال الأدوات المساعدة (Tooltips)
// ===========================
